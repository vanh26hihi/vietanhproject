...
class Boundary extends React.Component<{children:React.ReactNode;name:string},{error:Error|null}>{
  ...
  render(){if(this.state.error)return <div className="grid h-full place-items-center bg-background p-6"><div className="max-w-lg border bg-card p-6 text-center shadow-sm"><h2 className="text-base font-semibold">Module {this.props.name} gặp lỗi</h2><p className="mt-2 text-xs text-muted-foreground">Module khác vẫn hoạt động. Chuyển công cụ hoặc tải lại trang.</p><pre className="mt-3 overflow-auto border bg-muted p-2 text-left text-[10px]">{this.state.error.message}</pre></div></div>;return this.props.children;}
}
...
export default function Studio(){
  ...
  useEffect(()=>{const f=()=>setTool(fromHash());addEventListener("hashchange",f);return()=>removeEventListener("hashchange",f);},[]);
  const go=(t:Tool)=>{location.hash=t;setTool(t)};
  const meta=useMemo(()=>META[tool],[tool]);
  return <div className="studio-shell h-screen overflow-hidden bg-background text-foreground">
    <header className="studio-topbar">
      ...
      <div className="topbar-context">
        ...
        <span className="context-title">{meta.label}</span>
        <span className="context-separator"/>
        <span className="context-description">{meta.description}</span>
      </div>
      <div className="flex items-center gap-2 text-[10px] text-muted-foreground">
        <span className="status-chip"><CircleDot className="size-3 text-emerald-600"/>WebGL</span>
        <span className="status-chip"><ScanLine className="size-3"/>Z-up</span>
